...
def generate_spectrum_lines_suspicious_vector(spectrum_mutants_suspicious_vector, program_data_path):
    lines_and_pairs = []
    with open(os.path.join(program_data_path, 'mutants.log'), newline='', encoding='utf-8') as f:
        mutants = f.readlines()
        f.close()

    for mutants_suspicious_vector in spectrum_mutants_suspicious_vector:
        m_index = 0
        ...
        lines_and_pairs.append([lines_suspicious_score,class_line_pair_list])
        #return lines_suspicious_score, class_line_pair_list
    #assert len(lines_and_pairs) == 4
    return lines_and_pairs
