...
def result_show(project_id, bug_number, mode, spectrum_mode):
    ...
    if not os.path.exists(os.path.join(os.path.abspath('.'),
                                       'result',
                                       str(mode).lower(), #spectrum
                                       str(spectrum_mode))):# Op2
    ...
    for bug_id in range(bug_number):
        ...
        if not os.path.exists(os.path.join(os.path.abspath('..'), 'candidate-lines-result',
                                           str(mode).lower(),#spectrum
                                           str(spectrum_mode),# Op2
                                           str(project_id).lower(),#chart
                                           str(project_id) + str(bug_id + 1) + '.txt')):#Chart1.txt
        ...
        with open(os.path.join(os.path.abspath('..'), 'candidate-lines-result',
                               str(mode).lower(),#spectrum
                               str(spectrum_mode),#Op2
                               str(project_id).lower(),#chart
                               str(project_id) + str(bug_id + 1) + '.txt')) as candidate_file:#Chart1.txt
...
def result_display(project_id, project_num, mode):
    result_list = result_show(project_id, project_num, mode, spectrum_mode)
    return top_show(result_list, project_id, project_num)


if __name__ == '__main__':
    mode = 'spectrum'
    # spectrum_mode = 'Kulczynskil1'
    # spectrum_mode = 'Kulczynskil2'
    spectrum_mode = 'euclid'
    ...
    print('all_top1:{}'.format(all_top1))
    all_top3 = (chart_top3 + lang_top3 + math_top3 + mockito_top3 + time_top3 + closure_top3) / all_projects
    print('all_top3:{}'.format(all_top3))
    all_top5 = (chart_top5 + lang_top5 + math_top5 + mockito_top5 + time_top5 + closure_top5) / all_projects
    print('all_top5:{}'.format(all_top5))
    all_top10 = (chart_top10 + lang_top10 + math_top10 + mockito_top10 + time_top10 + closure_top10) / all_projects
    print('all_top10:{}'.format(all_top10))
    all_top50 = (chart_top50 + lang_top50 + math_top50 + mockito_top50 + time_top50 + closure_top50) / all_projects
    print('all_top50:{}'.format(all_top50))
    all_top100 = (chart_top100 + lang_top100 + math_top100 + mockito_top100 + time_top100 + closure_top100) / all_projects
    print('all_top100:{}'.format(all_top100))
    all_top200 = (chart_top200 + lang_top200 + math_top200 + mockito_top200 + time_top200 + closure_top200) / all_projects
    print('all_top200:{}'.format(all_top200))
